...
function codium_extend_header_style() {
    ...
    ?><style type="text/css">
        div#header {
            background: url(<?php header_image(); ?>); height :187px; -moz-border-radius-topleft:6px;border-top-left-radius:6px;-moz-border-radius-topright:6px;border-top-right-radius:6px;
        }
...
function codium_extend_posted_on() {
	printf( __( '<span class="%1$s"></span> %2$s <span class="meta-sep">von</span> %3$s.', 'codium_extend' ),
		...
		sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
			...
			sprintf( esc_attr__( 'Alle Beiträge zeigen von %s', 'codium_extend' ), get_the_author() ),
			get_the_author()
		)
	);
}
...
function codium_extend_posted_in() {
	...
	if ( $tag_list ) {
		$posted_in = __( 'Eintrag in %1$s veröffentlicht und mit %2$s getaggt. <a href="%3$s" title="Permalink to %4$s" rel="bookmark">Permalink</a> bookmarken.', 'codium_extend' );
	} elseif ( is_object_in_taxonomy( get_post_type(), 'category' ) ) {
		$posted_in = __( 'Eintrag in %1$s veröffentlicht. <a href="%3$s" title="Permalink to %4$s" rel="bookmark">Permalink</a> bookmarken.', 'codium_extend' );
	} else {
		$posted_in = __( '<a href="%3$s" title="Permalink to %4$s" rel="bookmark">Permalink</a> bookmarken.', 'codium_extend' );
	}
...
function codium_extend_comment($comment, $args, $depth) {
   ...
   <li id="comment-<?php comment_ID() ?>" <?php comment_class(); ?>>
      ...
      <?php if ($comment->comment_approved == '0') : ?>
         <em><?php _e('Dein Kommentar wartet auf die Freischaltung.', 'codium_extend') ?></em>
         <br />
      <?php endif; ?>
      ...
      <div class="comment-meta"><?php printf(__('%1$s - %2$s <span class="meta-sep">|</span> <a href="%3$s" title="Permalink to this comment">Permalink</a>', 'codium_extend'),
										...
										edit_comment_link(__('Bearbeiten', 'codium_extend'), ' <span class="meta-sep">|</span> <span class="edit-link">', '</span>'); ?></div>
	  <div class="clear"></div>	
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'codium_extend' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Bearbeiten', 'codium_extend' ), ' ' ); ?></p>
	<?php
			break;
	endswitch;
}
