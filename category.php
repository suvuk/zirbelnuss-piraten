	...
	<div id="container">
		<div id="content">
			<h2 class="page-title"><?php
					printf( __( '%s', 'codium_extend' ), '<span>' . single_cat_title( '', false ) . '</span>' );
			?></h2>
			...
			<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>> 
				<h2 class="entry-title"><a href="<?php the_permalink() ?>" title="<?php printf(__('Permalink zu %s', 'codium_extend'), esc_html(get_the_title(), 1)) ?>" rel="bookmark"><?php the_title() ?></a></h2>
				<div class="entry-date"><?php unset($previousday); printf(__('%1$s &#8211; %2$s', 'codium_extend'), the_date('', '', '', false), get_the_time()) ?></div>
									
						<div class="entry-content">
					<?php the_content(''.__('Weiterlesen <span class="meta-nav">&raquo;</span>', 'codium_extend').''); ?>
					<?php wp_link_pages("\t\t\t\t\t<div class='page-link'>".__('Pages: ', 'codium_extend'), "</div>\n", 'number'); ?>
						...
						<div class="entry-meta">
							...
							<span class="comments-link"><?php comments_popup_link(__('Noch keine Kommentare', 'codium_extend'), __('1 Kommentar', 'codium_extend'), __('% Kommentare', 'codium_extend')) ?></span>
						</div>
